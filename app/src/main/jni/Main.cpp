...
const char *title = OBFUSCATE("IL2cpp Tool By LuululaleW"); // Add your name here .
...
void draw_thread()
{
    ...
    if (ImGui::BeginTabBar("mainTabber"))
    {
        ...
        if (ImGui::BeginTabItem("Settings"))
        {
            ...
            if (ImGui::Button("Updates and stuff : https://t.me/LuululaleW", ImVec2(-1, 0)))
            {
                auto Application = Il2cpp::FindClass("UnityEngine.Application");
                auto OpenURL = Application->getMethod("OpenURL", 1);
                if (OpenURL)
                {
                    OpenURL->invoke_static<void>(Il2cpp::NewString(OBFUSCATE("https://t.me/LuululaleW")));
                }
                else
                {
